...
class UpActivity :  AppCompatActivity() , RobotLifecycleCallbacks {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_thumpsup)
        Handler().postDelayed({
            finish()
        }, 5000)
    ...
    override fun onRobotFocusGained(qiContext: QiContext?) {
        ...
        val animationG_1: Animation = AnimationBuilder.with(qiContext) // Create the builder with the context.
            .withResources(R.raw.salute_right_b001).build() // Set the animation resource.

        val animationG_2: Animation = AnimationBuilder.with(qiContext)
            .withResources(R.raw.nicereaction_a002).build()
